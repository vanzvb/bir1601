                                ...
                                <tbody>
                                    <form action="{{ route('test.encryptedbir') }}" method="GET">
                                        <tr>
                                            ...
                                            <td>
                                                <select id="bouID" name="bouID[]" class="form-select select2" multiple="multiple">
                                    ...
                                    <form action="{{ route('reports.bir1601plus') }}" method="GET">
                                        <tr>
                                            <td>BIR 1601 ++</td>
                                            <td>
                                                <select name="monthplus" id="monthplus" class="form-select select2" required>
                                                    {{-- <option value="">Select Month</option> --}}
                                                    @for ($m = 1; $m <= 12; $m++)
                                                        <option value="{{ $m }}">{{ date('F', mktime(0, 0, 0, $m, 1)) }}</option>
                                                    @endfor
                                            ...
                                            <td>
                                                <select name="yearplus" id="yearplus" class="form-select select2" required>
                                                    <option value="">Select Year</option>
                                                    @for ($y = 2029; $y >= 2019; $y--)
                                                    <option value="{{ $y }}">{{ $y }}</option>
                                                    @endfor
                                            ...
                                            <td>
                                                ...
                                                <select name="bouIDplus" id="BOUplus" class="form-select select2" >
                                                    {{-- <option value="">Select BOU</option> --}}
                                                    @foreach($company_bous as $company_bou)
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#bouID').select2({
                theme: 'bootstrap-5',
                width: '100%',
                placeholder: 'Select BOU',
                allowClear: true // Optional: This allows the user to clear the selection
            });
